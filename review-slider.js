document.addEventListener('DOMContentLoaded', function () {
    ...
    const updateCardWidths = () => {
        ...
        if (sliderDirection === 'vertical') {
            ...
            const availableHeight = 800 - 120;
            const cards = Math.min(cardsPerPage, 6);
            const cardHeight = (availableHeight - (gap * (cards - 1))) / cards;
            sliderBlock.style.setProperty('--card-height', `${cardHeight}px`);
        } else {
            // Ensure card height is consistent with the fixed container height
            sliderBlock.style.setProperty('--card-height', '100%');

            // Set the number of grid columns based on cardsPerPage (1-6)
            const cardsToShow = Math.max(1, Math.min(cardsPerPage, 6));
