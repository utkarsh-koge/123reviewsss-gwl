...
interface LayoutDisplaySectionProps {
    ...
    handleReviewsPerSlideBlur: () => void;
    gridRows: number;
    handleGridRowsChange: (value: string) => void;
    handleGridRowsBlur: () => void;
    gridColumns: number;
    handleGridColumnsChange: (value: string) => void;
    handleGridColumnsBlur: () => void;
    sectionBorderRadius: number;
    handleSectionBorderRadiusChange: (value: string) => void;
    handleSectionBorderRadiusBlur: () => void;
}

export const LayoutDisplaySection = (props: LayoutDisplaySectionProps) => {
    const {
        displayType, setDisplayType, displayTypeOptions,
        reviewsPerSlide, handleReviewsPerSlideChange, handleReviewsPerSlideBlur,
        gridRows, handleGridRowsChange, handleGridRowsBlur,
        gridColumns, handleGridColumnsChange, handleGridColumnsBlur,
        sectionBorderRadius, handleSectionBorderRadiusChange, handleSectionBorderRadiusBlur
    } = props;
            ...
            <InlineGrid columns={{ xs: '1fr', md: '1fr 2fr' }} gap="600" alignItems="start">
                ...
                <Card padding="400">
                    <BlockStack gap="400">
                        ...
                        {displayType === 'slider' && (
                            <TextField label="Reviews per slide" value={reviewsPerSlide === 0 ? '' : String(reviewsPerSlide)} onChange={handleReviewsPerSlideChange} onBlur={handleReviewsPerSlideBlur} type="number" min={1} max={6} helpText="Number of review cards visible at one time in slider (1-6)" autoComplete="off" />
                        )}

                        {displayType === 'grid' && (
                            <InlineGrid columns={{ xs: '1fr', sm: '1fr 1fr' }} gap="400">
                                <TextField label="Grid Rows" value={gridRows === 0 ? '' : String(gridRows)} onChange={handleGridRowsChange} onBlur={handleGridRowsBlur} type="number" min={1} max={6} helpText="Number of rows in the grid (1-6)" autoComplete="off" />
                                <TextField label="Grid Columns" value={gridColumns === 0 ? '' : String(gridColumns)} onChange={handleGridColumnsChange} onBlur={handleGridColumnsBlur} type="number" min={1} max={6} helpText="Number of columns in the grid (1-6)" autoComplete="off" />
                            </InlineGrid>
                        )}

                        <TextField label="Section Border Radius" value={sectionBorderRadius === 0 ? '' : String(sectionBorderRadius)} onChange={handleSectionBorderRadiusChange} onBlur={handleSectionBorderRadiusBlur} type="number" min={0} max={50} helpText="Border radius for the entire review section (0-50px)" autoComplete="off" suffix="px" />
